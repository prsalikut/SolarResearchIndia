...
class SolarFeatureEngineering:
    ...
    def add_solar_features(df):
        ...
        if temp_col in df.columns:
            df['temp_efficiency_factor'] = 1.0 - 0.004 * (df[temp_col] - 25)
...
def calculate_solar_power(df, panel_capacity_kw=5.0, efficiency=0.17):
    ...
    if 'is_monsoon' in df.columns:
        df['solar_power_kwh'] *= (1 - 0.4 * df['is_monsoon'])  # 40% reduction during monsoon
...
class SolarForecastingModel:
    ...
    def predict_future(self, historical_df, future_months, panel_capacity, efficiency, location_lat):
        ...
        results_df = pd.DataFrame({
            'date': future_dates,
            'predicted_power_kwh': np.maximum(predictions, 0),
...
def main():
    ...
    with tab1:
        ...
        if st.session_state.data is not None:
            ...
            st.plotly_chart(fig_historical, width='stretch', use_container_width=True)
            
            # Seasonal patterns
            fig_seasonal = plot_seasonal_patterns(df)
            if fig_seasonal:
                st.plotly_chart(fig_seasonal, width='stretch', use_container_width=True)
                    ...
                    with st.spinner("Training model..."):
                        try:
                            ...
                            if importance_df is not None:
                                ...
                                st.plotly_chart(fig_importance, width='stretch', use_container_width=True)
        ...
        if st.session_state.trained and st.session_state.model is not None:
            ...
            **Forecasting Setup:**  
            ...
            if st.session_state.forecast is not None:
                ...
                st.plotly_chart(fig_forecast, width='stretch', use_container_width=True)
                
                # Monthly summary
                st.subheader("Monthly Forecast Analysis")
                
                fig_summary, monthly_data = plot_forecast_summary(forecast_df)
                st.plotly_chart(fig_summary, width='stretch', use_container_width=True)
                
                # Display monthly data
                with st.expander("📋 View Monthly Forecast Data"):
                    st.dataframe(monthly_data, width='stretch', use_container_width=True)
