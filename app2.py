    ...
    if 'solar_irradiance_kwh_m2' in df.columns:
        fig.add_trace(
            go.Scatter(x=df['date'], y=df['solar_irradiance_kwh_m2'],
                      name='Solar Irradiance', line=dict(color='#2c3e50', width=1.5)),
            row=1, col=1
        )
    ...
    if 'cloudcover_mean' in df.columns:
        fig.add_trace(
            go.Scatter(x=df['date'], y=df['cloudcover_mean'],
                      name='Cloud Cover', line=dict(color='#7f8c8d', width=1.5)),
            row=3, col=1
        )
    
    fig.update_xaxes(title_text="Date", row=3, col=1, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_yaxes(title_text="kWh/m²", row=1, col=1, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_yaxes(title_text="°C", row=2, col=1, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_yaxes(title_text="%", row=3, col=1, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    
    fig.update_layout(
        height=700, 
        title_text=f"Weather Data - {location_name}",
        showlegend=True,
        plot_bgcolor='white',
        paper_bgcolor='white',
        font=dict(size=12)
...
def plot_seasonal_patterns(df):
    ...
    fig.add_trace(
        go.Bar(x=monthly_avg['year_month'], y=monthly_avg['solar_irradiance_kwh_m2'],
              name='Solar Irradiance', marker_color='#3498db'),
        row=1, col=1
    )
    ...
    if temp_col in monthly_avg.columns:
        fig.add_trace(
            go.Bar(x=monthly_avg['year_month'], y=monthly_avg[temp_col],
                  name='Temperature', marker_color='#e74c3c'),
            row=1, col=2
        )
    
    fig.update_xaxes(title_text="Month", tickangle=45, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_yaxes(title_text="kWh/m²", row=1, col=1, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_yaxes(title_text="°C", row=1, col=2, showgrid=True, gridwidth=0.5, gridcolor='#f0f0f0')
    fig.update_layout(
        height=400, 
        showlegend=False,
        plot_bgcolor='white',
...
def main():
    ...
    with tab5:
        ...
        - Analyze performance metrics (RMSE, MAE, R2, MAPE)
