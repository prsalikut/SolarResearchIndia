...
class SolarDataCollector:
    ...
    def generate_smart_synthetic_data(self, latitude, longitude, start_date, end_date):
        ...
        # Generate realistic solar data with minimum values
        solar_irradiance = np.maximum(base_irradiance + np.random.normal(0, 1.2, len(date_range)), 0.5)
        ...
        df['wind_speed_ms'] = 4 + 2 * np.random.random(len(date_range))
        
        # Clip values to realistic ranges
        df['solar_irradiance_kwh_m2'] = df['solar_irradiance_kwh_m2'].clip(0.5, 8)
        ...
        df['clear_sky_irradiance_kwh_m2'] = df['clear_sky_irradiance_kwh_m2'].clip(1, 10)
...
def calculate_solar_power(df, panel_capacity_kw=5.0, efficiency=0.17):
    ...
    # Apply efficiency factors if they exist
    ...
    # Apply monsoon reduction if applicable
...
class SolarFeatureEngineering:
    ...
    def add_temporal_features(df):
        ...
        # Seasonal classification for Indian context
    ...
    def add_solar_features(df):
        ...
        # Clearness index (actual/clear sky irradiance)
...
def plot_seasonal_decomposition(df):
    ...
    # Simple moving average for trend (30-day window)
...
def main():
    ...
    panel_capacity = st.sidebar.slider("Panel Capacity (kW)", 
                                       min_value=1.0, 
                                       max_value=100.0, 
                                       value=5.0, 
                                       step=0.5)
    ...
    st.sidebar.markdown(f"**System Size:** {panel_capacity} kW")
    st.sidebar.markdown(f"**Efficiency:** {panel_efficiency*100:.0f}%")
    ...
    # Main Content Tabs - Only 3 tabs now (removed Future Forecast)
    ...
    with tab1:
        ...
        with config_col3:
            st.metric("System", f"{panel_capacity} kW")
        ...
        if st.session_state.data is not None:
            ...
            if 'solar_power_kwh' in df.columns:
                ...
                with summary_col2:
                    st.metric("Standard Deviation", f"{df['solar_power_kwh'].std():.2f} kWh")
    ...
    with tab3:
        ...
        # Create tabs within documentation
        doc_tabs = st.tabs(["Overview", "Methodology", "Models", "Usage", "Technical"])
        ...
        with doc_tabs[0]:
            ...
            **Objective:** Develop hybrid predictive algorithms to forecast long-term solar power output in rural 
            ...
            1. **Multi-Model Approach:** Combines machine learning and statistical models
            2. **Location-Specific:** Tailored for rural Indian locations
            3. **Weather-Driven:** Incorporates solar irradiance, temperature, and cloud cover
            4. **Practical Application:** Supports solar infrastructure planning and policy development
            5. **User-Friendly Interface:** Intuitive Streamlit dashboard for easy interaction
            """)
        ...
        with doc_tabs[1]:
            ...
            - **Primary Source:** NASA POWER API for historical solar irradiance and weather data
            - **Fallback:** Realistic synthetic data generation based on location patterns
            - **Features:** Temporal, seasonal, weather variables with Indian context
            ...
            - **Solar-Specific:** Clearness index, temperature efficiency, cloud reduction
            - **Indian Context:** Monsoon season identification, regional patterns
            
            ### 3. Model Validation
            - **Time-Series Split:** Preserves temporal order (no lookahead bias)
            - **Comprehensive Metrics:** RMSE, MAE, R², MAPE for model comparison
        ...
        with doc_tabs[3]:
            ...
            1. **Select Location:** Choose from predefined rural Indian locations or enter custom coordinates
            2. **Set Data Period:** Define historical data collection range
            3. **Configure Solar System:** Set panel capacity and efficiency
            4. **Choose Models:** Select forecasting models to train
            
            ### Step 2: Data Collection (Tab 1)
            1. Click **"Collect Historical Data"** button
            2. Wait for data collection from NASA POWER API
            3. Review collected data and visualizations
            4. Download data if needed
            
            ### Step 3: Model Training (Tab 2)
            1. Configure training parameters (test size, random seed)
            2. Click **"Train Selected Models"** button
            3. Review model performance metrics
            4. Identify best performing models
            
            ### Tips for Best Results
            
            1. **Data Quality:** Collect at least 1 year of historical data for better seasonality capture
            2. **Model Selection:** Start with Random Forest and Gradient Boosting as baseline
            3. **System Configuration:** Use realistic panel efficiency values (15-20% for standard panels)
            """)
        ...
        with doc_tabs[4]:
            ...
            - **Backend:** Python with scikit-learn, statsmodels, pandas, numpy
            - **Visualization:** Plotly for interactive charts
            - **Data Storage:** Session state for temporary storage
            
            ### Data Sources
            - **Primary:** NASA POWER API (https://power.larc.nasa.gov)
            - **Features:** Solar irradiance, temperature, humidity, wind speed
            - **Resolution:** Daily data for long-term analysis
            ...
            1. **Data Availability:** Dependent on API availability and internet connectivity
            2. **Climate Stationarity:** Assumes future climate patterns similar to historical
            3. **Temporal Resolution:** Daily forecasts may not capture intra-day variations
            """)
        
        st.markdown("---")
        st.markdown("""
        <div style='text-align: center; color: #666; padding: 20px;'>
        <p>Developed for Solar Energy Forecasting Research in Rural India</p>
        <p>© 2024 Solar Forecasting Research Initiative</p>
        </div>
        """, unsafe_allow_html=True)
